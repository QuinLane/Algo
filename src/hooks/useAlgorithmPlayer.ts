...
export function useAlgorithmPlayer(trace: SortTrace | null) {
  ...
  const tick = useCallback(
    (timestamp: number) => {
      const delay = SPEED_MS[speed] ?? 100;
  ...
  const scrubTo = useCallback(
    (s: number) => {
      ...
      const clamped = Math.max(
        0,
        Math.min(s, (trace?.frames.length ?? 1) - 1)
      );
  ...
  // restart tick with new speed if playing
  useEffect(() => {
  ...
  const frame = trace?.frames[step] ?? null;
