...
import type { GraphTrace, GraphFrame } from "@/types/algorithm";
...
export function useGraphPlayer(trace: GraphTrace | null) {
  ...
  const tick = useCallback(
    (timestamp: number) => {
      const delay = SPEED_MS[speed] ?? 150;
  ...
  const pause = useCallback(() => { stopRaf(); setPlayerState("paused"); }, [stopRaf]);
  ...
  const frame: GraphFrame | null = trace?.frames[step] ?? null;
